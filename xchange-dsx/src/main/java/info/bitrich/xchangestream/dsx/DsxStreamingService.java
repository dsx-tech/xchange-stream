...
import info.bitrich.xchangestream.dsx.dto.DsxWebSocketBaseParams;
...
public class DsxStreamingService extends JsonNettyStreamingService {
    ...
    /**
     * Map request Id to Chanel Name and dsx method pair
     */
        ...
        if (message.has(JSON_ID)) {
            ...
            if (requests.containsKey(requestId)) {
                ...
                if (message.has(JSON_ERROR)) {
                    try {
                        DsxException exception = objectMapper.treeToValue(message, DsxException.class);
                    ...
                    LOG.info("dsx returned {} as result of '{}' method", result, subscriptionMethod);
                }
    ...
    private DsxWebSocketSubscriptionMessage generateSubscribeMessage(String channelName, String methodType) throws IOException {
        ...
        return new DsxWebSocketSubscriptionMessage(requestId, method, new DsxWebSocketBaseParams(symbol));
    }
}
